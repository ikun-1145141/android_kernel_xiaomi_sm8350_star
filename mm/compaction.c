...
/* Private per-node state keeps public MM structures ABI-stable for DLKMs. */
static bool proactive_compact_trigger[MAX_NUMNODES];
static bool proactive_compact_active[MAX_NUMNODES];
...
static enum compact_result __compact_finished(struct compact_control *cc)
{
	...
	if (READ_ONCE(proactive_compact_active[
			cc->zone->zone_pgdat->node_id])) {
...
static void proactive_compact_node(pg_data_t *pgdat)
{
	...
	WRITE_ONCE(proactive_compact_active[pgdat->node_id], true);
	for (zoneid = 0; zoneid < MAX_NR_ZONES; zoneid++) {
		struct zone *zone = &pgdat->node_zones[zoneid];
	...
	WRITE_ONCE(proactive_compact_active[pgdat->node_id], false);
}
		...
		int write, void __user *buffer, size_t *length, loff_t *ppos)
{
	...
	for_each_online_node(nid) {
		...
		if (READ_ONCE(proactive_compact_trigger[pgdat->node_id]))
			continue;
		WRITE_ONCE(proactive_compact_trigger[pgdat->node_id], true);
...
static inline bool kcompactd_work_requested(pg_data_t *pgdat)
{
	return pgdat->kcompactd_max_order > 0 || kthread_should_stop() ||
		READ_ONCE(proactive_compact_trigger[pgdat->node_id]);
}
...
static int kcompactd(void *p)
{
	...
	pgdat->kcompactd_max_order = 0;
	pgdat->kcompactd_classzone_idx = pgdat->nr_zones - 1;
	WRITE_ONCE(proactive_compact_trigger[pgdat->node_id], false);
